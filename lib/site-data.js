...
/* DİKKAT: Yorumlar yalnızca SAYFANIN İLÇESİYLE BİREBİR EŞLEŞEN müşterilerden
 * gösterilir. Başka ilçenin müşterisinin yorumu bu sayfada sergilenmez:
 * "{ilçe} ilçesinde bu hizmeti alan müşteriler" başlığı altında başka ilçeden
 * yorum göstermek yanıltıcıdır (önceden 2. aşamada herhangi bir ilçenin
 * yorumu ekleniyordu). Eşleşen yorum yoksa boş dizi döner ve sayfa yorum
 * bölümünü hiç göstermez. Sahte yorum ÜRETİLMEZ. */
...
export function getHizmetReferanslari(hizmetSlug, ilceSlug = '') {
  ...
  // Yalnızca birebir eşleşme: hizmet + ilçe (global listeden + hizmetin kendi verisinden)
  referanslar
    .filter((r) => r.hizmetSlug === hizmetSlug && refIlceSlug(r) === ilceSlug)
    .forEach(ekle);
  (h.referanslar || [])
    .filter((r) => refIlceSlug(r) === ilceSlug)
    .forEach(ekle);
